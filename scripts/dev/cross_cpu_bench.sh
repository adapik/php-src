...
#
# On Linux hosts, this builds and runs EVERYTHING inside a Docker container
# (ubuntu:24.04 + apt-installed toolchain) by default. That keeps the
# comparison apples-to-apples across an Ubuntu/Intel box and an AMD box --
# same base image, same package versions, same libc, same kernel-visible
# behavior, since Docker on Linux is namespaces/cgroups on the SAME host
# kernel, not a virtual machine.
#
# On macOS, this defaults to a NATIVE build instead (no Docker), because we
# found the opposite of what Docker gives you on Linux: Docker Desktop /
# OrbStack on macOS runs the container inside a hypervisor-virtualized
# Linux VM, not the host's own kernel. For this specific json-escaping
# workload (a tight byte-classification loop touching a small
# static/global table), that virtualization layer introduced a large,
# highly reproducible ~20-40% slowdown on Apple Silicon that was NOT
# present -- and in fact reversed into a speedup -- once measured with a
# genuinely native (non-virtualized) macOS build of the exact same source.
# Extensive isolated-kernel, disassembly, and address-shift experiments
# inside the container all failed to explain that slowdown from the code
# itself; it disappeared entirely outside the VM. So: trust native numbers
# on macOS, and treat any Docker-Desktop-derived macOS/arm64 numbers for
# this kind of memory-access-pattern-sensitive benchmark with suspicion.
# Override either way with USE_DOCKER=0/1; native builds need
# autoconf/automake/libtool/bison/re2c/pkg-config/a C compiler on the host
# (`brew install autoconf automake libtool bison re2c pkg-config` on macOS).
#
# Usage (download-and-run, on any Linux/macOS machine):
#   curl -fsSL <url-to-this-file> -o cross_cpu_bench.sh
#   chmod +x cross_cpu_bench.sh
#   ./cross_cpu_bench.sh                     # default: Docker on Linux, native on macOS; always fetches fresh master + fork branch
...
#   USE_DOCKER=1 ./cross_cpu_bench.sh        # force Docker even on macOS (not recommended -- see above)
#   USE_DOCKER=0 ./cross_cpu_bench.sh        # force native even on Linux
#
...
# Default: native on macOS (Docker Desktop/OrbStack there is a virtualized
# Linux VM and was found to distort exactly this kind of memory-access-
# pattern-sensitive benchmark -- see the header comment above), Docker on
# Linux (no VM layer there, so it stays the default for cross-machine
# toolchain consistency). Either is still overridable via USE_DOCKER.
...
#
# PIN_CMD is a plain string, not an array: "${PIN_CMD[@]}" on an
# empty-but-declared array is a genuine bash bug on versions before 4.4
# (spurious "unbound variable" under `set -u`) -- and macOS ships 3.2 by
# default (now the default build path on macOS too). taskset invocations
# never contain spaces/quoting that would need array-safety, so unquoted
# word-splitting below is fine.
PIN_CMD=""
...
# has to work under 3.2 too, now that native is the macOS default). A case
